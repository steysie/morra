...
All ***Morra*** taggers use hand crafter features. Now, it supports only `'RU'`
...
so your need to implement the successor of exactly `Feature2` class.

Then, just import your class and pass its name as the value for the
**features** param.

Besides the **features**, you can pass the helper methods for internal 
POS, LEMMA and FEATS classificators to the constructor. They serve to improve
...
script you can find an example of the real tagging pipeline where
`MorphParser3` object is created using those params.

**NB:** For prediction, you should create the parser object with the same
params as for the training. We save neither the feature class nor helper methods
with model.

### Loading the Training Data

After creating the parser, you need to load the traininig data.

```python
mp.load_train_corpus(corpus, append=False, parse=False, test=None, seed=None)
```
Here, **corpus** is a name of the file in *CONLL-U* format or a list/iterator of
...
here. In that case, the `.train()` method will be used.
...
and a specified part of it  will be stored as development *test corpus*. 
The remaining part will be stored as *train corpus*. Default is `None`: don't do that.
...
without validation. This will be faster but not as informative. And, you 
wouldn't be able to use autostop feature.
...
method, you should extract corpus statistics:
```python
...
For the training process description refer to the corresponding docs:
...
Apart from the separate methods for each *CONLL-U* field (refer to the
corresponding docs above), ***Morra*** has methods for predicting fields and
evaluating models conjointly. Note that you must already have trained models
for all taggers to use those methods.

#### Unidirectional Models

Predict the fields of only one sentence:
```python
...
the file in *CONLL-U* format or a list/iterator of sentences in
*Parsed CONLL-U*.

If **gold** is `None` (default), then loaded *test corpus* is used. If
**gold** is a ***Corpuscula***'s corpora wrapper, the `.test()` method will be
used.
...
the results of both base unidirectional taggers, choose one of the bases on the
ground of some heuristics.

**pos_repeats**: repeat the prediction step based on the previous one while
...
**feats_backoff**: if the results of bidirectional FEATS tagger differs from the
results of both base unidirectional taggers, choose one of the bases on the
ground of some heuristics.

**feats_repeats**: repeat the prediction step based on the previous one while
...
```
**feats_s_backoff**: if the result of separate FEATS-2 tagger differs from both
its base taggers, choose one of the bases on the ground of some heuristics (aka magic).

**feats_s_repeats**: the parameter for `predict_feats3()`

**feats_j_backoff**: if the result of joint FEATS-2 tagger differs from both its
base taggers, choose one of the bases on the ground of some heuristics.
...
```
All params where described earlier.
